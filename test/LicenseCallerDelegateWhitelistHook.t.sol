...
import { LicenseCallerDelegateWhitelistHook } from "../contracts/LicenseCallerDelegateWhitelistHook.sol";
import { BaseTest } from "@storyprotocol/periphery/test/utils/BaseTest.t.sol";

// Run this test:
// forge test --fork-url https://aeneid.storyrpc.io/ --match-path test/LicenseCallerDelegateWhitelistHook.t.sol
contract LicenseCallerDelegateWhitelistHookTest is BaseTest {
    ...
    LicenseCallerDelegateWhitelistHook public LICENSE_CALLER_WHITELIST_HOOK;
    ...
    function setUp() public override {
        ...
        LICENSE_CALLER_WHITELIST_HOOK = new LicenseCallerDelegateWhitelistHook(
            ACCESS_CONTROLLER,
    ...
    function test_LicenseCallerWhitelistHook_revert_addToWhitelistWhenAlreadyWhitelisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_AddressAlreadyWhitelisted.selector,
                bob
            )
        );
        vm.prank(alice);
    ...
    function test_LicenseCallerWhitelistHook_revert_addToWhitelistWhenNoPermission() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_NotOwnerOrDelegate.selector,
                bob,
                alice
            )
        );
    ...
    function test_LicenseCallerWhitelistHook_revert_removeFromWhitelistWhenNotInWhitelist() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_AddressNotInWhitelist.selector,
                bob
            )
        );
        vm.prank(alice);
    ...
    function test_LicenseCallerWhitelistHook_revert_beforeMintLicenseTokensWhenNotWhitelisted() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_AddressNotWhitelisted.selector,
                bob
            )
        );
    ...
    function test_LicenseCallerWhitelistHook_addDelegateSuccess() public {
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);

        assertTrue(LICENSE_CALLER_WHITELIST_HOOK.isDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob));
    }

    function test_LicenseCallerWhitelistHook_revert_addDelegateWhenAlreadyDelegate() public {
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);

        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_DelegateAlreadyAdded.selector,
                bob
            )
        );
        vm.prank(alice);
    ...
    function test_LicenseCallerWhitelistHook_revert_addDelegateWhenNoPermission() public {
        vm.expectRevert(); // AccessControlled will revert
        vm.prank(bob); // bob doesn't have permission for alice's IP
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie);
    }

    function test_LicenseCallerWhitelistHook_removeDelegateSuccess() public {
        // First add delegate
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);
        ...
        // Then remove
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.removeDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);

        assertFalse(LICENSE_CALLER_WHITELIST_HOOK.isDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob));
    }

    function test_LicenseCallerWhitelistHook_revert_removeDelegateWhenNotDelegate() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                LicenseCallerDelegateWhitelistHook.LicenseCallerWhitelistHook_DelegateNotFound.selector,
                bob
            )
        );
        vm.prank(alice);
    ...
    function test_LicenseCallerWhitelistHook_revert_removeDelegateWhenNoPermission() public {
        // First add delegate
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);
    ...
    function test_LicenseCallerWhitelistHook_isDelegateReturnsFalseByDefault() public {
        assertFalse(LICENSE_CALLER_WHITELIST_HOOK.isDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob));
    }

    function test_LicenseCallerWhitelistHook_delegateCanAddToWhitelist() public {
        // Alice adds bob as delegate
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);

        // Bob (delegate) adds charlie to whitelist
        vm.prank(bob);
        LICENSE_CALLER_WHITELIST_HOOK.addToWhitelist(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie);

        assertTrue(LICENSE_CALLER_WHITELIST_HOOK.isWhitelisted(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie));
    }

    function test_LicenseCallerWhitelistHook_delegateCanRemoveFromWhitelist() public {
        // Alice adds bob as delegate
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);

        // Alice adds charlie to whitelist
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addToWhitelist(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie);

        // Bob (delegate) removes charlie from whitelist
        vm.prank(bob);
        LICENSE_CALLER_WHITELIST_HOOK.removeFromWhitelist(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie);

        assertFalse(LICENSE_CALLER_WHITELIST_HOOK.isWhitelisted(ipId, address(PIL_TEMPLATE), licenseTermsId, charlie));
    }

    function test_LicenseCallerWhitelistHook_delegateIsolationBetweenLicenses() public {
        // Create a second license terms
        uint256 licenseTermsId2 = PIL_TEMPLATE.registerLicenseTerms(
            PILFlavors.commercialRemix({
                mintingFee: 200,
        ...
        assertTrue(LICENSE_CALLER_WHITELIST_HOOK.isDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob));
        assertFalse(LICENSE_CALLER_WHITELIST_HOOK.isDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId2, bob));
        ...
        vm.prank(bob);
        LICENSE_CALLER_WHITELIST_HOOK.addToWhitelist(ipId, address(PIL_TEMPLATE), licenseTermsId2, charlie);
    }

    function test_LicenseCallerWhitelistHook_removedDelegateCantManageWhitelist() public {
        // Alice adds bob as delegate
        vm.prank(alice);
        LICENSE_CALLER_WHITELIST_HOOK.addDelegate(ipId, address(PIL_TEMPLATE), licenseTermsId, bob);
        ...
        vm.prank(bob);
        LICENSE_CALLER_WHITELIST_HOOK.addToWhitelist(ipId, address(PIL_TEMPLATE), licenseTermsId, david);
    }

    function test_LicenseCallerWhitelistHook_ownerCanAlwaysManageWhitelist() public {
